...
export default function PrescriptionScan() {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch('https://mediwise-backend-production.up.railway.app/api/cabinet', {
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <button 
                          ...
                          onClick={async (e) => {
                            e.stopPropagation();
                            const token = localStorage.getItem('token');
                            if (!token) return toast.error('Please log in first to save medicines');
                            
                            try {
                              const response = await fetch('https://mediwise-backend-production.up.railway.app/api/cabinet', {
                                method: 'POST',
                                headers: {
                                  'Content-Type': 'application/json',
                                  'Authorization': `Bearer ${token}`
                                },
                                body: JSON.stringify({ medicine_name: med.name, dosage: med.dosage })
                              });
                              
                              const resData = await response.json();

                              if (!response.ok) {
                                if (resData.error && resData.error.includes('already in cabinet')) {
