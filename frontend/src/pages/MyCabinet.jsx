...
export default function MyCabinet({ user }) {
  ...
  const fetchCabinet = async () => {
    try {
      ...
      const medsData = data.medicines || [];
      medsData.sort((a, b) => new Date(b.added_at).getTime() - new Date(a.added_at).getTime());
      setMeds(medsData);
    } catch (error) {
      console.error(error);
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        if (data.error && data.error.includes('already in cabinet')) {
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://mediwise-backend-production.up.railway.app/api/cabinet/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
